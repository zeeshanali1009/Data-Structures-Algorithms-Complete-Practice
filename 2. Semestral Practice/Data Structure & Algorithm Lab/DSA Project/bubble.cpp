...
void bubbleSortAscending(Vehicle* head) {
    int n = 0;
    Vehicle* temp = head;
    while (temp != NULL) {
        n++;
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        for (int j = 0; j < n - 1 - i; j++) {
            if (temp->registrationNumber > temp->next->registrationNumber) {
                // Exchange values instead of using swap
                exchangeValues(*temp, *(temp->next));
            }
            temp = temp->next;
        }
    }
...
// Bubble sort function for sorting vehicles by registration number in descending order
void bubbleSortDescending(Vehicle* head) {
    int n = 0;
    Vehicle* temp = head;
    while (temp != NULL) {
        n++;
...
// Function to sort vehicles based on registration number
