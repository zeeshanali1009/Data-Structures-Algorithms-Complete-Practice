...
void insertionSortAscending(Vehicle* head) {
    ...
    while (current != NULL) {
        ...
        if (sorted == NULL || sorted->registrationNumber > current->registrationNumber) {
            current->next = sorted;
            sorted = current;
        } else {
            Vehicle* temp = sorted;
            while (temp->next != NULL && temp->next->registrationNumber < current->registrationNumber) {
                temp = temp->next;
            }
...
void insertionSortDescending(Vehicle* head) {
    if (head == NULL || head->next == NULL) {
        return;
    }
